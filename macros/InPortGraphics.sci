...
function [x,y,typ]=InPortGraphics(job,arg1,arg2)
//
x=[];y=[];typ=[];
select job
case 'plot' then
...
case 'set' then
  ...
  while %t do
    junction_name = "XCos_InPort"; // function name
    funtyp        = 5; // function type
    in            = [];// input port size
    it            = 1; // input port type
    ...
    depu          = 'n'; // direct feedthrough (y or n);
    dept          = 'n'; // time dependence (y or n);
  
    [ok,portname, porttype, out,lab]=..
    ...
    opar= list(portname, porttype, out);
    ot = 1;
...
case 'define' then
  ...
  label=[//junction_name;sci2exp(funtyp);
         ...
         //'n'
         "in0"; "TimedFloat"; "[1 1]"];
  gr_i=['xstringb(0,0,''GENERIC'',sz(1),sz(2),''fill'');']
